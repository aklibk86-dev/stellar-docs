...
![Stellar Theme 落地页](img/index.jpg)

### 用户仪表盘

集中展示账户、订阅、流量和快捷操作等常用信息。

![Stellar Theme 用户仪表盘](img/dashboard.jpg)

### 套餐页面

展示 XBoard 后端配置的可售套餐，用户可以选择套餐并进入结算流程。

![Stellar Theme 套餐页面](img/plans.jpg)
...
::: warning 开发环境配置注意
`api.example.com` 是保留的占位域名，无法连接真实 API。修改 `.env.development` 后**需要重启开发服务器**才能生效。生产环境 API 不读取此变量，请在 `public/env.js` 中配置。
:::
...
::: danger 代理安全警告
不要部署一个允许任意目标地址且没有白名单校验的开放代理，否则可能带来 **SSRF（服务端请求伪造）** 和带宽滥用风险。生产环境务必限制可代理的目标域名。
:::
...
::: tip 子目录部署关键
`window.routerBase` **必须**以 `/` 开头和结尾，正确写法如 `/stellar/`。配置错误会导致刷新子页面后出现 404，或登录跳转到错误路径。如果部署后 `env.js` 已上传但无法修改，可直接编辑服务器上的 `dist/env.js` 文件。
:::
...
::: tip 排查思路
遇到 API 请求失败，建议先在浏览器开发者工具（F12）的 Network 面板查看请求详情，确认是 CORS 错误、404 还是 502，这能快速定位问题方向。
:::
...
::: danger 重要
以下安全建议直接影响站点安全性，**部署前请逐条检查**。特别要注意前端配置文件对访问者完全可见，切勿在其中存放敏感信息。
:::
...
::: details 开发小贴士
- `@` 路径别名指向 `src/`
- Vue、Vue Router 和 Pinia 的常用 API 已自动导入，无需手动 import
- Naive UI 组件支持按需自动导入
- 新增页面后记得在路由表中注册
:::
